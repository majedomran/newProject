...
const LoginScreen = ({ navigation }) => {
  ...
  const { userID, userEmail, logedin, userPassword, loginError } = useSelector(
    (state) => state.auth
  );
  ...
  const signIn = async() => {
    ...
    await dispatch(getUserAuthAction());
    console.log(
  ...
  const { enterEmail, error, login, signUp, email, enterPassword, password } =
    lang.en;
  return <View style={{ flex: 1 }}>{loginRender()}</View>;
};
